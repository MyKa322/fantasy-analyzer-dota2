...
**Матч** — разбор любой игры по её id прямо из OpenDota, с очками компендиума за карту;
...
нетворт, урон, варды) она показывает то, что статистическому сервису считать незачем: сколько
очков компендиума стоит эта карта каждому игроку на лучшем нейтральном баннере его роли и какие
Coaching Titles сработали бы на этой карте и на ком. Роли выводятся из матча — мид по лейну,
Support Duo по двум наименьшим нетвортам, — и страница об этом говорит прямо. Разбор любой карты
это ссылка: `#/match/8922016200`, и дата в таблице матчей профиля — она же.
...
frontend/                         React + TS + Tailwind + Recharts
  ...
  src/opendota.ts                 клиент OpenDota для страницы матча
  src/components/MatchPanel.tsx   разбор матча: таблица, очки компендиума, титулы
...
`profiles.json` (~2 МБ) — он грузится только при заходе на вкладку «Профили». Вкладке «Матч» не
нужен ни тот, ни другой: она спрашивает OpenDota напрямую. Вкладки, которым нужен живой сервер
(«Данные», «Рейтинги», ручной конструктор), в этом режиме скрыты.
