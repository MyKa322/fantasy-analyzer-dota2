...
заранее и лежит в `frontend/public/data/snapshot.json` (~320 КБ), а математика эмблем повторена
на TypeScript в `frontend/src/engine/scoring.ts` — поэтому подбор баннера, ограничение по роллам
и рейтинги по статам работают прямо в браузере. Вкладки, которым нужен живой сервер («Данные»,
«Рейтинги», ручной конструктор), в этом режиме скрыты.
