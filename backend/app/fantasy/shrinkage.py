...
где `se^2 = s^2 / n_eff` — квадрат стандартной ошибки среднего команды, а `tau^2`
...
Почему `n_eff`, а не число карт
-------------------------------
Оценка команды — не простое среднее, а взвешенное по свежести (полураспад 30
дней). Точность взвешенного среднего задаётся не количеством карт, а тем,
насколько неравномерны веса: `n_eff = (Σw)² / Σw²`. На реальных данных 134 карты
за 180 дней дают n_eff ≈ 72, и так у всех: номинал завышает выборку в 1,5–2,3
раза.

Подстановка числа карт занижала `se^2` примерно вдвое, `lambda` уезжала к
единице, и поправка работала вполсилы — сильнее всего недожимая как раз те
маленькие выборки, ради которых она написана. С правильным знаменателем мид
Team Resilience с 37 картами уходит с первого места, куда его ставили 17
эффективных карт, а не игра.
...
def shrink_to_role_mean(
    ...
    for positions in groups.values():
        ...
        # Эффективный размер выборки, а не число карт: оценка взвешена по
        # свежести (см. заголовок модуля). Откат на число карт — для значений,
        # посчитанных без весов: там взвешивания нет и n_eff равен n.
        games = np.array(
            [
                result[t][v].effective_games or result[t][v].games
